    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          <label>Day</label>
          <div class="day-display">{{ dayLabel }}</div>
        </div>
...
const emit = defineEmits<{
  save: [data: { startTime: number; endTime: number }];
  cancel: [];
  delete: [];
}>();
...
const dayLabel = computed(() => dayLabels[props.dayOfWeek]);
...
function convertFrom24Hour(minutes: number): { hour: number; minute: number; ampm: string } {
  const totalHours = Math.floor(minutes / 60);
  const mins = minutes % 60;
  
  if (totalHours === 0) {
    return { hour: 12, minute: mins, ampm: 'AM' };
  } else if (totalHours < 12) {
    return { hour: totalHours, minute: mins, ampm: 'AM' };
  } else if (totalHours === 12) {
    return { hour: 12, minute: mins, ampm: 'PM' };
  } else {
    return { hour: totalHours - 12, minute: mins, ampm: 'PM' };
  }
}
...
onMounted(() => {
  const start = convertFrom24Hour(props.window.startTime);
  startHour.value = start.hour;
  startMinute.value = start.minute;
  startAmPm.value = start.ampm;

  const end = convertFrom24Hour(props.window.endTime);
